...
internal data class BehovssekvensId private constructor(private val value: String) {
    init { requireNoException(error = "$value er ikke en gyldig BehovssekvensId", block = {ULID.parseULID(value)})}
    override fun toString() = value
    internal companion object {
...
internal data class OmsorgspengerSaksnummer private constructor(private val value: String) {
    init { require(value.matches(Regex)) { "$value er et ugyldig omsorgspenger saksnummer" } }
    override fun toString() = value
    internal companion object {
